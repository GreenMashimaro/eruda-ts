...
export class ResourceStorageBase {
  private _devTools: DevTools
  private _dataGrid!: LunaDataGrid
  private _selectStorageKey: string | null = null
  ...
  constructor($container: $.$, devTools: DevTools, storage: Storage, title: string) {
  ...
  private _triggerEventCopyStorage() {
    const selectStorageKey = this._selectStorageKey
    if (selectStorageKey === null) {
      return
    }

    const value = this._storage.getItem(selectStorageKey) || ''
    copy(value)
  ...
  private _triggerEventDeleteStorage() {
    ...
    this._deleteStoreDataByKey(selectStorageKey)
    this.refresh()
  }

  private _triggerEventDataGridSelect(node: { data: { key: string } }) {
    this._selectStorageKey = node.data.key
    this._updateButtons()
  }

  private _triggerEventDataGridDeselect() {
    this._selectStorageKey = null
    this._updateButtons()
  }
  ...
  private _updateButtons() {
    ...
    if (this._selectStorageKey === null) {
